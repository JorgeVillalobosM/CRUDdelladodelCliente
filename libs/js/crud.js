    ...
    $(document).one('click','button[type="button"]', function(event){
        ...
        db.transaction(function(transaction){
            ...
            transaction.executeSql(sql, undefined, function(){
                alert("Registro borrado satisfactoriamente, actualice la página")
...
$(function () {
    ...
    //Listar e imprimir la tabla en la página web
                ...
                if (result.rows.length) {
                    ...
                    for (var i = 0; i < result.rows.length; i++) {
                        ...
                        $("#listaProductos").append('<tr id="fila' + id + '" class="Reg_A' + id + '"><td><span class="mid">A' +
                            ...
                            precio + '</span></td><td><button type="button" id="A'+id+'" class="btn btn-success" onclick="editar()"><img src="libs/img/icon.png"/></button></td><td><button type="button" id="A'+id+'" class= "btn btn-danger" onclick="eliminarRegistro()"><img src="libs/img/delete.png"/></button></td></tr>');
                    }
                } else {
    ...
    $("#borrarTodo").click(function(){
        if(!confirm("¿Está seguro de borrar la tabla?, los datos se eliminarán permanentemente",""))
        return;
        db.transaction(function(transaction){
            var sql="DROP TABLE productos";
            transaction.executeSql(sql,undefined,function(){
                alert("Tabla eliminada de forma satisfactoria, actualice la página")
            }, function(transaction, err){
                alert(err.message);
            })
        })
